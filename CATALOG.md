...
|-------|------|----------|--------|---------|------------------|-------------|
| `adversarial-review` | `skills/general/adversarial-review/` | `general` | Active | Codex, Claude, Cursor, and Gemini enabled | `deep`; heavy mode `ultracode` for Claude-only UltraCode runs | Script-backed portable control plane for deterministic adversarial review of specs and plans, with attested direct adapters or first-class generic task bundles, immutable IDs, revise/reject resolution, provenance, and resumable reports. |
