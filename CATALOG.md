...
|-------|------|----------|--------|---------|------------------|-------------|
| `adversarial-review` | `skills/general/adversarial-review/` | `general` | Active | Codex, Claude, Cursor, and Gemini enabled | `deep`; heavy mode `ultracode` for Claude-only UltraCode runs | Script-backed portable control plane for deterministic review of specs and plans. The public CLI emits first-class generic bundles for host-native execution; fixture-conformant direct adapter components remain gated on orchestrator-supplied dispatch evidence. Includes immutable IDs, revise/reject resolution, provenance, and resumable reports. |
